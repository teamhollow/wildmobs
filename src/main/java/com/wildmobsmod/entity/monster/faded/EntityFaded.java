...
{
	//
	// This mob is quite interesting. It's essentially a zombie that has two held item slots, one with a weapon, and one with a random item it can use to heal or apply a potion effect. They also travel in groups,
	// following an automatically chosen leader.
	// The mob only spawns during rain. It should have a custom drop, but I haven't decided what it should be (Something that fits their pack behavior)
	//
	
	private int eating;
	private int eatingSound;
	
	// Backpack is just a funny name for the fadeds that run slower than other fadeds when attacking. If a pack has more than four fadeds, some of them are made to move slower to avoid them clustering together.
	public boolean isInBackpack;
	
    private static final UUID backPackSlonwessUUID = UUID.fromString("3d17be0f-fc6d-4947-8d90-0e0556f1b737");
    private static final AttributeModifier backPackSlonwessModifier = new AttributeModifier(backPackSlonwessUUID, "Back pack slowness", -0.25D, 1);
	
	public EntityFaded(World p_i1745_1_)
	{
		super(p_i1745_1_);
	...
	public void writeEntityToNBT(NBTTagCompound p_70014_1_)
	{
		...
		p_70014_1_.setTag("SecondaryHandItem", nbttagcompound2);
		p_70014_1_.setFloat("SecondaryHandItemDropChance", this.getHandEquipmentDropChance());
		
		p_70014_1_.setBoolean("IsLeader", this.getIsLeader());
	}
	...
	public void readEntityFromNBT(NBTTagCompound p_70037_1_)
	{
		...
		this.setHandEquipment(ItemStack.loadItemStackFromNBT(p_70037_1_.getCompoundTag("SecondaryHandItem")));
		this.setHandEquipmentDropChance(p_70037_1_.getFloat("SecondaryHandItemDropChance"));
		
        this.setIsLeader(p_70037_1_.getBoolean("IsLeader"));
	}
	...
	public void setHandEquipment(ItemStack p_82229_1_)
	{
		this.dataWatcher.updateObject(15, p_82229_1_);
	}
	...
	public void setHandEquipmentDropChance(float p_82229_1_)
	{
		this.dataWatcher.updateObject(16, p_82229_1_);
	}
    ...
    public void setIsLeader(boolean p_70900_1_)
    {
        if (p_70900_1_)
        {
	...
	protected void dropEquipment(boolean p_82160_1_, int p_82160_2_)
	{
		...
		if (itemstack != null && (p_82160_1_ || flag1) && this.rand.nextFloat() - (float)p_82160_2_ * 0.01F < this.getHandEquipmentDropChance())
		{
		...
		if (!this.worldObj.isRemote)
		{
            ...
            if  (this.eating > 0)
            {
            	if (this.eating == 1)
				{
					...
					{
						this.addPotionEffect(new PotionEffect(MainRegistry.aquaHealingID, 20 * 90, 0));
					}
    ...
    {
    	if ((this.worldObj.getWorldInfo().isRaining() || this.worldObj.getWorldInfo().isThundering()) && this.worldObj.provider instanceof WorldProviderSurface && this.worldObj.canBlockSeeTheSky(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY), MathHelper.floor_double(this.posZ)))
    	{
    		List list = this.worldObj.getEntitiesWithinAABB(this.getClass(), this.boundingBox.expand(8.0D, 4.0D, 8.0D));
